"""Capture C1.2 responsive panel screenshots at 360/420/520px (offscreen).

Usage (from the worktree root, using its venv):
    .\\.venv\\Scripts\\python.exe scripts\\capture_frontend_c12_panel_widths.py

The CreativeAgentPanel is rendered with a fully seeded Mock timeline at fixed
panel widths; each capture asserts the timeline and the card container exist
and are visible before saving.
"""
...
def main() -> int:
    app = QGuiApplication([])
    app.setApplicationName("AI Novel Studio C1.2 Screenshot")
    ...
    engine.loadData(
        ...
        """,
        QUrl.fromLocalFile(str(app_qml_path().parent / "c12-harness.qml")),
    )
    if not engine.rootObjects():
        print("panel harness failed to load", file=sys.stderr)
        return 1
    ...
    for width in (520, 420, 360):
        ...
        if panel is None or timeline is None:
            raise AssertionError(f"panel/timeline missing at {width}px")
        if panel.property("visible") is not True or timeline.property("visible") is not True:
            raise AssertionError(f"panel/timeline not visible at {width}px")
        target = out_dir / f"c1.2-panel-{width}px.png"
        saved = window.grabWindow().save(str(target))
        if not saved:
            raise AssertionError(f"failed to save {target.name}")
        print(f"OK   {target.name} (panel {panel.width()}px)")

    print("All C1.2 panel screenshots captured.")
    return 0
